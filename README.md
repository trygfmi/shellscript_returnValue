...
<p>このリポジトリは、シェルスクリプトを実行して出力結果を取得し、ファイルの中身を表示します</p>
</div>
...
<div class="wp-block-group">
<p>上記のコマンドをインストール済みの方は、以下のコマンドを実行してリポジトリからダウンロード後、ディレクトリを移動し、シェルスクリプトを実行して返り値で受け取ったファイル名から内容を表示してみてください</p>
